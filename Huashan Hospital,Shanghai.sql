...
DETERMINISTIC
BEGIN
	RETURN FLOOR(DATEDIFF('2025-12-31',date_of_birth)/365.25);
END$$

DELIMITER ;

SELECT age(date_of_birth) AS age, COUNT(*) AS Number_of_patients FROM patients
...
SELECT CONCAT(FLOOR(DATEDIFF('2025-12-31',date_of_birth)/10/365.25)*10,'s')  AS Age_group,gender, COUNT(*)Total_patients FROM patients
...
SELECT insurance_provider, ROUND(AVG(age(date_of_birth)),2) AS Average_age FROM patients
GROUP BY insurance_provider;
